...
from ultralytics.utils.torch_utils import strip_optimizer, select_device, time_sync
from ultralytics.utils.checks import check_imgsz, check_requirements, check_imshow
...
class Detect:
    ...
    def detect(opt, save_img=False):
        ...
        webcam = source.isnumeric() or source.endswith('.txt') or source.lower().startswith(('rtsp://',
                                                                                             'rtmp://',
                                                                                             'http://',
                                                                                             'https://'))
        ...
        imgsz = check_imgsz(imgsz, stride=stride)  # check imgsz
        if half:
            model.half()  # to FP16
        ...
        if webcam:
            ...
            dataset = LoadStreams(source, imgsz=imgsz, vid_stride=stride)
        else:
            save_img = True
            dataset = LoadImages(source, imgsz=imgsz, vid_stride=stride)
