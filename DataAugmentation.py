...
class DataAugmentation:
    def __init__(self, image_dir, annotation_dir, output_dir, train_file):
    ...
    def runner(self):
        ...
        for image_name in os.listdir(self.image_dir):
            ...
            image, bboxes = self.readImgAndAnnotations(image_path, annotation_path)

            for i in range(5):  # Augment each image 5 times
                augmented_image, augmented_bboxes = self.applyAugmentation(image, bboxes)

                # Save the augmented image and annotations
                self.saveAugmentedData(
                    augmented_image, augmented_bboxes, self.output_dir, image_name, annotation_name
                )
    ...
    def readImageAndAnnotations(img_path, ann_path):
        ...
        with open(ann_path, "r") as fp:
            ...
            for line in lines:
                ...
                cid, x, y, width, height = map(float, line.strip().split())
                bboxes.append([cid, x, y, width, height])
    ...
    def applyAugmentation(img, bboxes):
        ...
        aug_image = DataAugmentation.applyRotation(img, angle)

        # Randomly apply scaling (scale factor between 0.8 to 1.2)
        scale_factor = np.random.uniform(0.8, 1.2)
        aug_image = DataAugmentation.applyScaling(aug_image, scale_factor)
        ...
        aug_image = DataAugmentation.adjustBrightness(aug_image, brightness_factor)
